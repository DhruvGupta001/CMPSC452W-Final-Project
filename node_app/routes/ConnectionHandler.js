const io = require('socket.io')(); // Import the Socket.IO library
...
let ConnectionHandler = function(redisClient, io) {
            ...
            (err) => {
                this.redisClient.sadd('connections', mac,
                    (err, reply) => {
                        ...
                        let logMessage = new Date().toLocaleString() + ": " + alias + " connected";
                        this.io.sockets.emit('log_message', logMessage);
    ...
    this.removeConnection = (req, res) => {
        ...
        redisClient.hset(mac, 'connected', '0', (err, reply) => {
            ...
            // Transmit disconnection to web app's websocket
            io.sockets.emit('disconnection', {
                mac: mac
            });
            // Log the disconnection message
            let logMessage = new Date().toLocaleString() + ": " + mac + " disconnected";
            io.sockets.emit('log_message', logMessage);
