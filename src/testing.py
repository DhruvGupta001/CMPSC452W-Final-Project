...
disconnect_time = 1800  # time to wait before the client is considered disconnected
...
__printing = False  # flag to indicate if the script prints output
connected_file = "resources/connected_file.json"  # file to log connections
log_file = "/src/resources/unknown_connections.log"  # log file for unknown devices
...
def load_macs(filename):
    """
    Loads devices and their corresponding MAC addresses from a given JSON file
    """
    try:
        global mac_defs
...
def check_connected(gateway_ip, ip_range):
    global mac_defs, connected, __checking, black_list, __printing
...
def check_for_disconnections():
    ...
    """
    global connected, mac_defs, disconnect_time, __broadcast, __printing
    ...
    for each in connected:
        ...
        if __printing:
            print(OKBLUE + "{:<20}{:<20}".format(mac_defs[each], str(diff)) + ENDC)
...
def broadcast_connection(mac, alias, timestamp, ip):
    """
    sends an HTTP POST request with the MAC address of a newly connected
...
def broadcast_disconnection(mac):
    """
    sends an HTTP DELETE request with the MAC address of a newly disconnected
    device to a specified URL
    """
    try:
        global web_service_url
...
if __name__ == "__main__":
    ...
    # # Check for root access on Unix-based systems
    # if os.getuid() != 0:
    ...
    print("monitoring connections to the network...")
